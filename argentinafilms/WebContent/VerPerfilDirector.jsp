...
<c:import url="/ServletVerPerfilDirector" /> <!-- llamo servlet al cargar pagina -->


<c:if test="${not empty directorSelect}">
...
<div class="contenido">
	<div class="posters film-list clear posters-70 film-details-list no-title" style="margin-left:30%; width:40%;">      
		...
		<div class="pull-left"> 
			<h2 class="film-title prettify">${directorSelect.nombre} ${directorSelect.apellido} (Director)</h2> 
	...
	<section id="live-feed" class="section" style="margin-left:20%; width:60%; margin-top:50px;">
		<h2 class="section-heading">Peliculas en las que fue director</h2>
		
		<c:if test="${lasPeliculas.size() == 0 || lasPeliculas == null}">
			<div style="font-size: 18px!important; color: #A9E2F3; font-style:italic">(Este director no figura en ninguna película)</div>
		</c:if>
...
<c:if test="${empty directorSelect}">
