...
<c:import url="/ListarPeticionesDirectores" /> <!-- llamo servlet al cargar pagina -->
...
<c:if test="${userLogueado.rango == 'admin' || userLogueado.rango == 'moderador'}">
	...
	<div class="login">
		<h1 class="text-center titulo-seccion"><img src="imgs/RegistrarDirector.png" class="imagen-seccion" />ACEPTAR DIRECTORES</h1>
	</div>
		...
		<table class="table" style="font-color: yellow!important; font-size: 14px!important; width: 900px!important; margin: auto;"> 
		    ...
		    <tbody>
		   		 ...
		   		 <c:if test="${peticionesDirectores.size() == 0 || peticionesDirectores == null}">
					<tr><td colspan="5" align="center">No hay peticiones de directores.</td></tr>
				</c:if>
		    
				<c:forEach items="${peticionesDirectores}" var="peticionDirector">
						<tr id="fila${peticionDirector.id_director}">
						<td>${peticionDirector.nombre}</td>
						<td>${peticionDirector.apellido}</td>
						<td>${peticionDirector.edad}</td>

						<td style="width:200px!important">
                        <button type="button" onclick="javascript:AceptarDirector(${peticionDirector.id_director});" class="btn btn-success btn-sm" style="margin: auto!important">Aceptar</button>                         
                        <button type="button" onclick="javascript:RechazarDirector(${peticionDirector.id_director});" class="btn btn-danger btn-sm" style="margin: auto!important">Rechazar</button>
                    	</td>
					</tr>
				</c:forEach>
			</tbody>
